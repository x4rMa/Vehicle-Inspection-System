		...
		<div id="app">
			...
			<div class="sidebar app-aside" id="sidebar">
				<div class="sidebar-container perfect-scrollbar">
								...
								<ul class="sub-menu">
									...
									<li>
										<a href="{{ URL::to('pendingpaid') }}">
											<span class="title"> Vehicles (Paid)  <span class="badge">{{ Vehicle::where('user_id','=', Auth::id())->doesntHave('certificate')->where('paid', true)->count() }}</span></span>
										</a>
									</li>

									<li>
										<a href="{{ URL::to('pendingunpaid') }}">
											<span class="title"> Vehicles (Unpaid)  <span class="badge">{{ Vehicle::where('user_id','=', Auth::id())->doesntHave('certificate')->where('paid', false)->count() }}</span></span>
										</a>
									</li>
								...
								<ul class="sub-menu">
									...
									<li>
										<a href="{{ URL::to('complete') }}">
											<span class="title"> Issue Certificate 
												<span class="badge"> {{ DB::table('vehicles')
			            ...
			            $join->on('vehicles.id', '=', 'vehicles_inspection.vehicle_id')
			                 ->where('vehicles_inspection.status', '=', true);
			        })->count() - Vehicle::has('certificate')->count() }}
 												</span> </span>
										</a>
									</li>
									@endif
									...
									<li>
										<a href="{{ URL::to('vehicles/certificates/0') }}">
											<span class="title"> Approve Certificates <span class="badge">{{ Certificate::where('status','=', 0)->count() }}</span> </span>
										</a>
									</li>
									<li>
							...
							<li>
								...
								<ul class="sub-menu">
									 <li>
										<a href="{{ URL::to('pendingunpaid') }}">
											<span class="title"> Pending (Unpaid) <span class="badge">{{ Vehicle::doesntHave('certificate')->where('paid', false)->count() }}</span> </span>
										</a>
									</li>
									<li>
										<a href="{{ URL::to('vehiclesinspection') }}">
											<span class="title"> Pending (Paid) <span class="badge">{{ DB::table('vehicles')
			        ...
			        ->count() }}</span> </span>
									...
									<li>
										<a href="{{ URL::to('complete') }}">
											<span class="title"> Completed <span class="badge"> {{ DB::table('vehicles')
					->leftJoin('users', 'vehicles.user_id', '=', 'users.id')
			        ->join('vehicles_inspection', function($join)
			        {
			            $join->on('vehicles.id', '=', 'vehicles_inspection.vehicle_id')
							...
							<li>
								<a href="javascript:void(0)">
									<div class="item-content">
										...
										<div class="item-inner">
											<span class="title"> PDF Reports </span><i class="icon-arrow"></i>
										</div>
									</div>
								</a>
								<ul class="sub-menu">
									@if( !Auth::user()->isGeneralUser() )
									<li>
										<a href="{{ URL::to('pdfreports/create') }}">
											<span class="title">Upload New PDF Report</span>
										</a>
									</li>
									@endif
									<li>
										<a href="{{ URL::to('pdfreports') }}">
											<span class="title"> All PDF Reports <span class="badge">{{ PDFReports::all()->count() }}</span></span>
										</a>
									</li>
						...
						<ul class="nav navbar-right">
							...
							<li class="dropdown current-user">
								<a href class="dropdown-toggle" data-toggle="dropdown">
									<img src="http://www.esmeth.com/wp-content/uploads/2014/11/user_placeholder-370x276.png" alt="user"> <span class="username">@if(Auth::check()) {{ Auth::user()->username; }} @else Guest @endif <i class="ti-angle-down"></i></i></span>
								</a>
				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<section id="page-title" class="padding-top-15 padding-bottom-15">
							<div class="row">
								<div class="col-sm-9">
									<h1 class="mainTitle">EAAS - Japan : {{ $title }} </h1>
